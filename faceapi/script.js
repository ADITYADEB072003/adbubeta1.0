...
async function getLabeledFaceDescriptions() {
  const labels = [ "adi", "Dc2022bca0014_kash"];
  
  // Map each label to a promise that resolves to LabeledFaceDescriptors
  const labeledDescriptorsPromises = labels.map(async (label) => {
    const descriptions = await Promise.all([loadFaceDescriptor(label, 1)]); // Load descriptor for the first image
