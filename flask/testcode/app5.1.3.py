...
def log_recognized_face(student_id, student_name):
    ...
    with open(recognized_faces_csv, 'a', newline='') as csvfile:
        ...
        print(f"Logged {student_id}, {student_name}, {timestamp}")
...
def read_recognized_faces_csv():
    data = []
    if os.path.exists(recognized_faces_csv):
        with open(recognized_faces_csv, 'r') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                data.append(row)
        print(f"Read data from CSV: {data}")  # Debug statement
    else:
        print(f"CSV file does not exist at path: {recognized_faces_csv}")
    return data

@app.route('/')
def index():
    recognized_faces_data = read_recognized_faces_csv()
    print(f"Data passed to template: {recognized_faces_data}")  # Debug statement
    return render_template('index4.html', recognized_faces=recognized_faces_data)
